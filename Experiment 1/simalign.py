...
class EmbeddingLoader(object):
    def __init__(self, model: str="bert-base-multilingual-cased", device=torch.device('cuda:0'), layer: int=8):
        TR_Models = {
...
class Simalign:
    def __init__(self, layer, matching_methods, model: str="xlmr", token_type: str="bpe", distortion: float=0.0, null_align: float=1.0, device: str="cuda:0"):
        model_names = {
        ...
        LOG.info("Simalign parameters: model=%s; token_type=%s; distortion=%s; null_align=%s; matching_methods=%s; device=%s" % (model, token_type, distortion, null_align, matching_methods, device))
    ...
    def align_sentences(self, source_sentences, target_sentences, batch_size=100):
        ...
        # --------------------------------------------------------
        if self.null_align < 1.0:
            entropies = {x: [] for x in self.matching_methods}
            for sent_id in range(len(original_corpora[0])):
                sent_pair = [original_corpora[i][sent_id] for i in [0, 1]]
                vectors = self.embed_loader.get_embed_list(sent_pair).cpu().detach().numpy()
                ...
                methods_matrix = {}
                methods_matrix["forward"], methods_matrix["backward"] = self.get_alignment_matrix(sim)
                methods_matrix["inter"] = methods_matrix["forward"] * methods_matrix["backward"]
                if "mwmf" in self.matching_methods:
                    methods_matrix["mwmf"] = self.get_max_weight_match(sim)
                if "itermax" in self.matching_methods:
                    methods_matrix["itermax"] = self.iter_max(sim)

                for m in entropies:
                    entropies[m] += self.gather_null_aligns(sim, methods_matrix[m])
            null_thresh = {m: sorted(entropies[m])[int(self.null_align * len(entropies[m]))] for m in entropies}
        ...
        for batch_id, batch_sentences in enumerate(tqdm(data_loader)):
            ...
            for in_batch_id, sent_id in enumerate(batch_sentences[0].numpy()):
                ...
                sim = self.apply_distortion(sim, self.distortion)
                if self.null_align < 1.0:
                    mask_nulls = {mmethod: self.apply_percentile_null_aligns(sim, null_thresh[mmethod]) for mmethod in self.matching_methods}
                ...
                if self.null_align < 1.0:
                    if "inter" in self.matching_methods:
                    ...
                    for j in range(len(vectors[1])):
                        ...
                        if all_mats[ext][i, j] > 0:
                            ...
                            if self.token_type == "bpe":
                                b2w_aligns.add('{}-{}'.format(sentences_b2w_map[sent_id][0][i], sentences_b2w_map[sent_id][1][j]))
                                b2w_scores['{}-{}'.format(sentences_b2w_map[sent_id][0][i], sentences_b2w_map[sent_id][1][j])].append(sim[i, j])
                                if ext == "inter":
